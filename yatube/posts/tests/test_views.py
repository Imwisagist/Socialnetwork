...
class FollowTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        Follow.objects.create(user=cls.user_follower, author=cls.user_following)
        cls.post = Post.objects.create(
    ...
    def test_subscription_feed(self):
        ...
        self.assertEqual(Follow.objects.all().count(), self.ONE)
        response = self.client_follower.get(reverse("posts:follow_index", None))
        post = response.context["page_obj"][0].text
        self.assertEqual(post, self.post.text)
        response = self.client_following.get(reverse("posts:follow_index", None))
